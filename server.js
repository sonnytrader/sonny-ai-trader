// server.js (ANA PROJE - V12.9 - TÜM HATALAR GİDERİLDİ + 1M HIZLI MOMENTUM)
// SÜRÜM: V12.9 (1m Momentum, Tüm hatalar giderildi) (27.10.2025)
...
console.log("--- server.js dosyası okunmaya başlandı (V12.9 - 1 DAKİKALIK HIZLANDIRILMIŞ MOMENTUM) ---");
...
        const avgVolume = volumeSum / validVolumeCount; if(isNaN(avgVolume) || avgVolume <= 0) return null; 
        ...
        // KRİTİK DÜZELTME 3: volumeStatusText tanımlaması (ReferenceError hatasını giderir)
...
    } catch (error) { console.error("Kritik 15m Tarama Hatası:", error.message); } finally { 
        ...
        // Momentum sinyali temizleme süresi ayara bağlandı:
        const momentumTemizlemeZamani = Date.now() - (MOMENTUM_COOLDOWN_MS * 2); // Momentum cooldown'ın 2 katı (10 dk)
        
...
            }
            if (s.id && s.id.endsWith('-MOMENTUM')) { 
...
            if (analysisResult) { 
                analysisResult.strategyType = 'Momentum'; 
                global.APP_STATE.signals.unshift(analysisResult); 
                io.emit('yeni_sinyal', {...analysisResult, strategyType: 'MOMENTUM'}); // Socket.IO'ya sinyalin tipini ekleyerek gönder
            } 
            
            // KRİTİK ÇÖZÜM 1: API kısıtlamasını önlemek için gecikme eklendi.
            await new Promise(resolve => setTimeout(resolve, MOMENTUM_API_DELAY_MS));
            
        } catch (loopError) { 
            // Konsola aşırı yüklenmemek için bu hata basımını kapatıyorum. Hata CCXT'nin içinden zaten yönetiliyor.
            // console.error(`[Momentum Tarama Döngü Hatası (${ccxtSymbol})]: ${loopError.message}`); 
        }
    } 
}
...
    } catch(err) { 
        // KRİTİK ÇÖZÜM 2: Sözdizimi hatası (', err:', fazlalığı) giderildi.
        console.error("Manuel analiz API hatası:", err.message, err.stack); 
        res.status(500).json({ error: `Sunucu hatası: ${err.message}` }); 
    }
});
server.listen(PORT, async () => {
    console.log("=============================================="); console.log(`🚀 Sonny AI Trader (V12.9 - 1 DAKİKALIK HIZLANDIRILMIŞ MOMENTUM) http://localhost:${PORT}`); console.log(`OTOMATİK TARAMA BAŞLIYOR...`); 
