// server.js (ANA PROJE - V12.10 - TÜM HATALAR GİDERİLDİ + 1M HIZLI MOMENTUM)
// SÜRÜM: V12.10 (1m Momentum, Tüm hatalar giderildi, Breakout undefined fix) (28.10.2025)
...
console.log("--- server.js dosyası okunmaya başlandı (V12.10 - 1 DAKİKALIK HIZLANDIRILMIŞ MOMENTUM) ---");
...
// === V1.0 MOMENTUM PATLAMASI AYARLARI === // KRİTİK 1M AYARLARI
const MOMENTUM_TIMEFRAME = '1m'; // <<< 1m olarak ayarlandı
const MOMENTUM_LOOKBACK = 20;
const MOMENTUM_SCAN_INTERVAL = 10 * 1000; // 10s tarama aralığı
const VOLUME_SPIKE_MULTIPLIER = 5.0; // Sinyal yakalama kolaylığı için 5.0'a düşürüldü
const PRICE_SPIKE_PERCENT = 0.5; // Sinyal yakalama kolaylığı için 0.5'e düşürüldü
...
// 🚀 KRİTİK ÇÖZÜM: CCXT HATA GİDERİCİ GECİKME (Momentum için)
const MOMENTUM_API_DELAY_MS = 250; // Her API çağrısı arasında 250ms bekle. (CCXT için güvenli)
...
// 💡 CCXT Rate Limit Artırıldı (Render için daha güvenli)
const exchange = new ccxt.bitget({ 'enableRateLimit': true, 'rateLimit': 200 }); // 100'den 200'e çıkarıldı
...
        const bbWidthPercentValue = (typeof middleBand === 'number' && middleBand !== 0 && typeof upperBand === 'number' && typeof lowerBand === 'number') ? (((upperBand - lowerBand) / middleBand) * 100) : 0;
...
            bbWidth: bbWidthPercentValue.toFixed(2)
        };
...
        if (!ohlcv || ohlcv.length < requiredCandles) return null;
        ...
        // Piyasa koşulunu kontrol et
        const marketCondition = await checkMarketCondition(ccxtSymbol);

        // ✅ KRİTİK DÜZELTME: marketCondition tanımsızsa varsayılan trend ata
...
        const avgVolume = volumeSum / validVolumeCount; if(isNaN(avgVolume) || avgVolume <= 0) return null;
        ...
        // ✅ KRİTİK DÜZELTME: volumeStatusText tanımlaması
        const isVolumeConfirmed = lastVolume >= avgVolume * BREAKOUT_VOLUME_MULTIPLIER;
        const isBalinaConfirmed = lastVolume >= avgVolume * 2.0;
        const volumeStatusText = isBalinaConfirmed ? `BALİNA TEYİTLİ` : (isVolumeConfirmed ? `Yüksek` : `Düşük`);
...
        if (signal !== 'WAIT' && !isFiltered) { console.log(`\x1b[36m>>> V12.6 2h KIRILIM SİNYALİ: ${resultData.symbol} - ${resultData.signal} (Güven: ${resultData.confidence}%)\x1b[0m`); return resultData; }
        else { return null; }
...
                bbWidth: bbWidthPercentValue.toFixed(2), reason: `Hacim: ${volumeMultiplier.toFixed(1)}x, Fiyat Değ: ${priceChangePercent.toFixed(2)}%`,
                strategyType: 'Momentum' // Strateji tipini ekle
            };
...
        } else { return null; }
    } catch (error) {
        // console.error(`[Momentum Tarama Döngü Hatası (${ccxtSymbol})]: ${error.message}`); // Hata çok sık tekrarlandığı için kapatıldı
        return null;
    }
}
...
    } catch (error) { console.error("Kritik 15m Tarama Hatası:", error.message); } finally {
        ...
        const momentumTemizlemeZamani = Date.now() - (MOMENTUM_SIGNAL_LIFESPAN); // Ayara bağlandı

        global.APP_STATE.signals = global.APP_STATE.signals.filter(s => {
            if (!s || !s.timestamp) return false; // Eksik sinyalleri temizle
            if (s.strategyType === 'Momentum') {
                return s.timestamp > momentumTemizlemeZamani;
            } else { // StochEMA veya Breakout
...
        try {
            const analysisResult = await analyzeMomentumSpike(ccxtSymbol);
            if (analysisResult) {
                // Sinyali genel listeye ekle (strategyType zaten içinde var)
                global.APP_STATE.signals.unshift(analysisResult);
                // ✅ DÜZELTME: Tüm sinyalleri tek bir olayla gönder
                io.emit('yeni_sinyal', analysisResult);
            }

            // KRİTİK ÇÖZÜM: API kısıtlamasını önlemek için gecikme eklendi.
            await new Promise(resolve => setTimeout(resolve, MOMENTUM_API_DELAY_MS));

        } catch (loopError) {
            // console.error(`[Momentum Tarama Döngü Hatası (${ccxtSymbol})]: ${loopError.message}`); // Aşırı loglamayı önle
        }
    }
}
...
    } catch(err) {
        ...
        console.error("Manuel analiz API hatası:", err.message, err.stack);
        res.status(500).json({ error: `Sunucu hatası: ${err.message}` });
    }
});
server.listen(PORT, async () => {
    console.log("=============================================="); console.log(`🚀 Sonny AI Trader (V12.10 - 1 DAKİKALIK HIZLANDIRILMIŞ MOMENTUM) http://localhost:${PORT}`); console.log(`OTOMATİK TARAMA BAŞLIYOR...`);
...
    setInterval(runMomentumScan, MOMENTUM_SCAN_INTERVAL);
});
