// server.js (ANA PROJE V5.1 - Stoch+EMA Tam Sürüm)
// SÜRÜM: V5.1 (15m Stoch+EMA, Dinamik Güven, Sıcak Liste Kaldırıldı) (25.10.2025)
...
console.log("--- server.js dosyası okunmaya başlandı (V5.1 - Stoch+EMA Tam Sürüm) ---");
...
const MIN_VOLUME_MULTIPLIER = 1.0; // Güven puanı için Hacim eşiği (1.0x)
...
const SIGNAL_COOLDOWN_MS = 30 * 60 * 1000; // 15m sinyalleri için 30dk cooldown makul
let signalCooldowns = {};
let globalWatchlist = {};
let globalTargetList = []; // Sıcak Liste kaldırıldığı için sadece Hedef Liste kaldı
...
/**
 * AŞAMA 1 - HIZLI ÖN TARAYICI (Sıcak Liste kaldırıldı, sadece Hedef Liste)
 */
...
/**
 * ANA STRATEJİ: Geliştirilmiş Stoch+EMA (15m) - DİNAMİK GÜVEN PUANI
 */
async function analyzeStochEMACoin(ccxtSymbol, isManual = false, isWatchlist = false) {
    ...
    try {
        ...
        if (avgVolume && lastVolume) {
             volumeStatus = `Hacim: ${(lastVolume / avgVolume).toFixed(1)}x`;
        }
        ...
        if (signal !== 'WAIT') {
            ...
            if ((signal === 'LONG' && stochOversold) || (signal === 'SHORT' && stochOverbought)) {
                confidenceScore += 5;
                reason += ' [Stoch Aşırı Bölge]';
            }
        }

        // R/R 2.5'u uygula ve BB Genişlik Kontrolü
        ...
        const forecastLevel = signal !== 'WAIT'
            ? calculateFibonacciExtension(ohlcv, BOLLINGER_PERIOD, signal)
            : null;
...
server.listen(PORT, async () => {
    console.log("==============================================");
    console.log(`🚀 Sonny AI Trader (V5.1 - Stoch+EMA Tam Sürüm) http://localhost:${PORT}`);
