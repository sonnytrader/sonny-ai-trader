...
function compressionInfo(candles) {
    ...
    return {
        ratio: num(ratio, 3),
        rangePct: num(rangePct, 3),
        good: ratio <= CONFIG.COMPRESSION_MAX_ATR_RATIO
    };
}
...
async function getOI(symbol) {
    ...
    try {
        ...
        if (CONFIG.DEBUG) {
            console.log(`[OI] ${symbol}: ${error.message}`);
        }
        return null;
    }
}
...
async function analyze2H(symbol) {
    ...
    try {
        ...
        const longTrigger = structure.resistance * (1 + CONFIG.BREAKOUT_BUFFER);
        const shortTrigger = structure.support * (1 - CONFIG.BREAKOUT_BUFFER);
        ...
        if (longDistance >= 0 && longDistance <= CONFIG.WATCH_DISTANCE_PERCENT) {
            direction = 'LONG';
            trigger = longTrigger;
            breakoutLevel = structure.resistance;
        } else if (shortDistance >= 0 && shortDistance <= CONFIG.WATCH_DISTANCE_PERCENT) {
            direction = 'SHORT';
            trigger = shortTrigger;
            breakoutLevel = structure.support;
        }
        ...
        const risk = buildRisk(candles, direction, trigger, structure);
        if (!risk) {
        ...
        if (existing) {
            ...
            existing.resistance = structure.resistance;
            existing.support = structure.support;
...
async function checkClosed2HBreakouts() {
    ...
    for (const setup of watchSetups) {
        try {
            ...
            const structure = buildStructure(previousLookback);
            ...
            if (setup.direction === 'LONG') {
                ...
                if (!breakoutConfirmed && high > longTrigger) {
                    ...
                    logReject(setup.symbol, 'FALSE_BREAKOUT_WICK');
                }
            } else {
            ...
            if (volumeRatio < CONFIG.MIN_VOLUME_RATIO) {
                ...
                DEBUG.lowVolume++;
                logReject(setup.symbol, 'LOW_VOLUME');
                continue;
            }
...
async function finalizeSignal(setup) {
    ...
    if (rr < CONFIG.MIN_RR) {
        ...
        logReject(setup.symbol, 'RR_TOO_LOW');
        return;
    }
    ...
    if (targetPercent < CONFIG.TARGET_SPACE_MIN_PERCENT) {
        ...
        logReject(setup.symbol, 'TARGET_TOO_CLOSE');
        return;
    }
...
async function run2HScan() {
    ...
    APP_STATE.scanStatus = {
        message: `2H yapı taraması: ${targets.length} coin`,
        isScanning: true
    };
    ...
    APP_STATE.scanStatus = {
        message: `2H tarama tamamlandı | ${setups.length} setup`,
        isScanning: false
    };
...
<span class="legend">
YEŞİL DESTEK • KIRMIZI DİRENÇ • SARI ÜÇGEN / SIKIŞMA
</span>
</div>
<div class="chart">
<canvas id="cv"></canvas>
...
function render(){
    ...
    if(!setups.length){
        list.innerHTML='<div style="padding:20px;color:#7f8b98">Şu an WATCH / BREAKOUT setup yok.</div>';
        return;
    }
    list.innerHTML=setups.map(s=>\`
<div class="card \${selected&&selected.id===s.id?'sel':''}" data-id="\${esc(s.id)}">
<div class="row">
<span class="coin">\${esc(s.symbol.replace(':USDT',''))}</span>
<b class="\${s.direction==='LONG'?'long':'short'}">\${s.direction}</b>
</div>
<div style="margin-top:5px">
<span class="badge">\${esc(s.state)}</span>
\${s.triangle?'<span class="badge tri">▲ ÜÇGEN / SIKIŞMA</span>':''}
</div>
<div class="meta">
...
\`).join('');
    ...
    document.querySelectorAll('.card').forEach(card=>{
        card.onclick=()=>{
            selected=setups.find(s=>s.id===card.dataset.id)||null;
            render();
...
function drawTrendLine(ctx,line,X,Y,visibleCount,offset,type){
    ...
    const startIndex=0;
    const endIndex=visibleCount-1;
    const startGlobal=startIndex+offset;
    const endGlobal=endIndex+offset;
    const y1=Y(line.slope*startGlobal+line.intercept);
    const y2=Y(line.slope*endGlobal+line.intercept);
    ctx.save();
...
function draw(){
    ...
    const visible=chart.candles.slice(-60);
    const count=visible.length;

    let minPrice=Math.min(...visible.map(c=>Number(c[3])));
    let maxPrice=Math.max(...visible.map(c=>Number(c[2])));

    const structure=chart.structure;

    if(structure){
        [structure.resistance,structure.support,
         structure.resistanceLine?.current,structure.supportLine?.current,
         structure.resistanceLine?.projected,structure.supportLine?.projected]
        .forEach(value=>{
            if(value&&value<minPrice)minPrice=value;
            if(value&&value>maxPrice)maxPrice=value;
        });
    }
    ...
    const LEFT=50,RIGHT=115,TOP=15,BOTTOM=18;
    ...
    for(let i=0;i<=5;i++){
        ...
        ctx.stroke();
        ctx.fillText(fmt(maxPrice-(maxPrice-minPrice)*i/5),5,y+3);
    }

    const candleWidth=Math.max(2,Math.min(9,PW/count*.65));
    ...
    visible.forEach((candle,index)=>{
        ...
        const color=bullish?'#17d7a0':'#ff5c77';
    ...
    if(structure){
        const offset=chart.candles.length-count;
        ...
        if(structure.triangle&&structure.resistanceLine&&structure.supportLine){
            ...
            const startGlobal=offset;
            const endGlobal=offset+count-1;
            ...
            ctx.fillText('SIKIŞMA / ÜÇGEN',LEFT+8,TOP+15);
            ctx.restore();
        }
    ...
    if(selected){
        ...
        document.getElementById('details').innerHTML=values.map(item=>\`
<div class="d">
<span>\${item[0]}</span>
<b>\${esc(item[1])}</b>
</div>
...
async function loadChart(symbol){
    try{
        const response=await fetch('/api/chart?symbol='+encodeURIComponent(symbol)+'&timeframe=2h');
        const data=await response.json();
        if(data.success){
            chart=data;
            document.getElementById('title').textContent=
                symbol.replace(':USDT','')+' — 2H DESTEK / DİRENÇ / SIKIŞMA';
            draw();
        }
    }catch(error){
        console.error(error);
...
function apply(data){
    ...
    if(selected){
        selected=setups.find(s=>s.id===selected.id)||selected;
    }
    render();
    draw();
    document.getElementById('status').textContent=
        (data.scanStatus?.message||'Hazır')+' | '+new Date().toLocaleTimeString('tr-TR');
}
...
function connect(){
    ...
    ws.onclose=()=>{
        document.getElementById('status').textContent='Bağlantı yenileniyor...';
        setTimeout(connect,3000);
    };
}

fetch('/api/setups').then(r=>r.json()).then(apply).catch(()=>{});
connect();
...
async function start() {
    try {
        ...
        setInterval(() => { void runAll(); }, CONFIG.SCAN_INTERVAL_MS);
        setInterval(() => { void updateLivePricesAndRetests(); }, CONFIG.LIVE_INTERVAL_MS);
        setInterval(() => { void runPreScan(); }, CONFIG.PRESCAN_INTERVAL_MS);

        console.log('SONNY 2H döngüleri başlatıldı.');
    } catch (error) {
