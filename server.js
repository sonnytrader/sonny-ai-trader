// server.js (ANA PROJE - V12.7 - RENKLİ MOMENTUM + BBW - EKSİKSİZ)
// SÜRÜM: V12.7 (15m %BBW Gösterimi, R/R Bonusu *1, BBW Filtresi Gevşetildi, V1.0 Momentum Eklendi) (27.10.2025)
...
console.log("--- server.js dosyası okunmaya başlandı (V12.7 - Renkli Momentum + BBW - EKSİKSİZ) ---"); // Sürüm no güncellendi
...
// === V12.7 STRATEJİ AYARLARI === // Sürüm no güncellendi
...
// === V1.0 MOMENTUM PATLAMASI AYARLARI === // YENİ (V12.7 Entegrasyonu)
const MOMENTUM_TIMEFRAME = '1m';
const MOMENTUM_LOOKBACK = 20; // Ortalama hacim için kaç mum geriye bakılacak (1m * 20 = 20dk)
const MOMENTUM_SCAN_INTERVAL = 15 * 1000; // Tarama sıklığı (15 saniye)
const VOLUME_SPIKE_MULTIPLIER = 10.0; // Hacim patlaması eşiği (Ortalamanın X katı)
const PRICE_SPIKE_PERCENT = 1.0; // Fiyat patlaması eşiği (%)
const MOMENTUM_BB_PERIOD = 20; // Volatilite (%BBW) hesaplaması için Bollinger periyodu (1m * 20 = 20dk)
const MOMENTUM_COOLDOWN_MS = 5 * 60 * 1000; // Aynı coinden tekrar sinyal gelmesi için bekleme süresi (5 dk)


let signalCooldowns = {};
let globalWatchlist = {};
let globalTargetList = [];
let momentumCooldowns = {}; // YENİ (V12.7 Entegrasyonu)
...
async function analyzeStochEMACoin(ccxtSymbol, isManual = false, isWatchlist = false) {
    ...
    try {
        ...
        // DEBUG LOGları yorum satırı yapıldı
        ...
        if (signal !== 'WAIT') {
            ...
            if (!isFiltered) {
                 // <<< DÜZENLEME V12.6: BBW Filtresi Gevşetildi >>>
            ...
            if (!isFiltered) {
                ...
                // <<< DÜZENLEME (V12.6): R/R Bonusu * 1 >>>
                confidence += (rrRatio * 1); // Eski: * 0.5
                // <<< DÜZENLEME SONU >>>
...
/** YENİ STRATEJİ (1m): V1.0 - Momentum Patlaması (Pump/Dump) */ // YENİ (V12.7 Entegrasyonu)
...
// YENİ Momentum Tarama Fonksiyonu (V12.7 Entegrasyonu)
async function runMomentumScan() {
    // const scanTime = new Date(); const scanTimeStr = scanTime.toLocaleTimeString(); // Çok sık log ürettiği için kapalı
    if (globalTargetList.length === 0) return; const allSwapSymbols = [...globalTargetList];
    for (const ccxtSymbol of allSwapSymbols) { if (!ccxtSymbol) continue; try { const analysisResult = await analyzeMomentumSpike(ccxtSymbol); if (analysisResult) { analysisResult.strategyType = 'Momentum'; global.APP_STATE.signals.unshift(analysisResult); io.emit('yeni_momentum_sinyali', analysisResult); } await new Promise(resolve => setTimeout(resolve, API_DELAY_MS / 4)); } catch (loopError) { /* console.error(`[1m Momentum Döngü Hatası (${ccxtSymbol})]: ${loopError.message}`); */ }}
    // console.log(`--- 1m MOMENTUM TARAMA TAMAMLANDI (${scanTimeStr}). ---`); // Çok sık log ürettiği için kapalı
}
...
server.listen(PORT, async () => {
    console.log("=============================================="); console.log(`🚀 Sonny AI Trader (V12.6 - R/R*1 + BBW) http://localhost:${PORT}`); console.log(`OTOMATİK TARAMA BAŞLIYOR...`); // Sürüm no güncellendi
    ...
    setInterval(runMomentumScan, MOMENTUM_SCAN_INTERVAL); // YENİ (V12.7 Entegrasyonu)
});
console.log("--- server.js dosyası okunması tamamlandı ---"); // Bu son satır (Satır ~706)